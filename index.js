...
function Manager () {
  this.getHooksConf = function (hook_type, hook_name, cb) {
    var glob = require('glob')
      , confs = [];
    if (typeof hook_name !== "function") {
      var conf_file = path.join('hooks', hook_type, hook_name, 'hook.json');
    ...
    fs.stat(hook_type + '.d', function(err, stat) {
      ...
      glob(path.join('hooks', hook_type, '*', 'hook.json'), function (err, files) {
        if (err) {
          return cb(err);
        }
        if (files.length < 1) {
...
Manager.prototype = {
  ...
  hook: function (hook_type) {
    ...
    process.stdin.on('end', function () {
      var pid = daemon.start('stdout.log', 'stderr.log');
      ...
      glob(path.join(hook_type + '.d', '*'), function (err, files) {
        ...
        files.forEach(function (filename) {
          ...
          var child_proc = child_process.execFile(filename, [], {}, function (err) {
            if (err) {
          ...
          console.log('execute ' + filename + ' (' + child_proc.pid + ')');
  ...
  search: function (hook_type, query) {
    var hooks = [];
    module.exports.getHooksConf(hook_type, function (err, confs) {
      if (err) {
        return console.error(err);
      }
  ...
  add: function (hook_type, hook_name) {
    this.getHooksConf(hook_type, hook_name, function (err, conf) {
      if (err) {
        return console.error(err);
      }
      var filepath = path.join(hook_type + '.d', hook_name + '.index');
      if (fs.existsSync(filepath)) {
        return console.error(hook_name + ' already exists for the ' + hook_type + ' hook');
      }
      function setup () {
        fs.symlinkSync(path.join('..', 'hooks', hook_type, hook_name, conf.index), path.join(hook_type + '.d', hook_name + '.index'));
  ...
  rm: function (hook_type, hook_name) {
    this.getHooksConf(hook_type, hook_name, function (err, conf) {
      ...
      var directory = path.join(hook_type + '.d', hook_name)
        , index = path.join(hook_type + '.d', hook_name + '.index');
